...
class Student:
    ...
    def __init__(self, root):
        ...
        main_frame = Frame(
            bg_img, bd=2
        )  # bd = border, bg_img is put in as frame is over it
        ...
        )  # relief is border style
        Left_frame.place(x=10, y=10, width=730, height=580)

        img_left = PIL.Image.open(r"./img/princeton.jpg")
        img_left = img_left.resize(
            (720, 130), PIL.Image.LANCZOS
        )  # ANTIALIAS converts high level img to low level
        self.photoimg_left = PIL.ImageTk.PhotoImage(
            img_left
        )  # variable goes inside parameter'''
        ...
        dep_label = Label(
            ...
            font=("times new roman", 12, "bold"),
            bg="white",
        )  # department label is dep
        dep_label.grid(row=0, column=0, padx=10)  # row and column start from 0
        self.dep_combo = ttk.Combobox(
            current_course_frame,
            font=("times new roman", 12, "bold"),
            state="read only",
        )  # fancy combo box from ttk, requires frame inside parameter; state read only makes it read only
        ...
        self.dep_combo.current(
            0
        )  # gives current position of first value in the above tuple, iterates through
        ...
        self.grade_combo = ttk.Combobox(
            current_course_frame, font=("times new roman", 12, "bold")
        )
        ...
        self.studentId_entry = ttk.Entry(
        ...
        self.studentName_entry = ttk.Entry(
            class_Student_frame, width=20, font=("times new roman", 13, "bold")
        )  # entry box for typing
        ...
        take_photo_btn = Button(
            ...
            bg="blue",
            cursor="hand2",
            command=self.take_photo_samples_fn,
        )  # collect_samples(studentName_entry.get()))
        ...
        self.photoimg_right = PIL.ImageTk.PhotoImage(
            img_right
        )  # variable goes inside parameter
        ...
        search_entry = ttk.Entry(
            Search_frame, width=14, font=("times new roman", 13, "bold")
        )  # , textvariable = search_var) #entry box for typing
        ...
        search_btn = Button(
            ...
            cursor="hand2",
            command=lambda: [
                self.showAll_btn.config(state=NORMAL),
                self.searching_info(search_combo.get(), search_entry.get()),
            ],
        )
        search_btn.grid(row=0, column=3, padx=5)

        self.showAll_btn = Button(
            Search_frame,
            text="Show All",
            width=12,
            bg="blue",
            cursor="hand2",
            command=self.fetching_info,
        )  # lambda: [self.fetching_info(), showAll_btn.config(state = DISABLED)])
        ...
        )  # scrollbar horizontal component
        scroll_y = ttk.Scrollbar(
            table_frame, orient=VERTICAL
        )  # scrollbar vertical component
        ...
        )  # Treeview to display hierarchy of items with attributes
        ...
        self.student_table.column(
            "Stream", width=100
        )  # access one heading at a time with respective heading as a parameter
