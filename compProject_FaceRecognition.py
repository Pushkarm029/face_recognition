...
# 				img_dir = os.path.dirname(os.path.realpath("./student_imgs"))
...
def collect_samples(sample_name, student_id):
    ...
    while True:
        ...
        )  # OpenCV uses BGR format, whereas face_recognition uses RGB.
        ...
        for (top, right, bottom, left), face_encoding in zip(
            ...
            img_dir = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "Sample_Imgs"
            )

            if cv2.waitKey(1) & 0xFF == ord("y"):
                img_dir = os.path.join("./student_imgs")
                img_path = os.path.join(
                    img_dir, "{}_{}_sample_img.jpg".format(sample_name, student_id)
                )
            ...
            cv2.rectangle(
                frame, (left, bottom - 30), (right, bottom), (214, 119, 30), -1
            )
            cv2.putText(
                frame,
                "Face detected - press 'y'",
                (left + 5, bottom - 5),
...
def load_images():
    ...
    try:
        img_dir = "./student_imgs/"  # Student images directory.
        onlyfiles = [f for f in listdir(img_dir) if isfile(join(img_dir, f))]
        ...
        for i in onlyfiles:
            if i[::-1][:15][::-1] == "_sample_img.jpg":
                ...
                student_id = i.split("_")[1]
                sample_face_names.append(sample_name2 + "_" + student_id)
                sample_img2 = face_recognition.load_image_file(
                    "{}{}_{}_sample_img.jpg".format(img_dir, sample_name2, student_id)
                )

                try:
                    sample_face_encoding2 = face_recognition.face_encodings(
                        sample_img2
                    )[0]
                    sample_face_encodings.append(sample_face_encoding2)
                except IndexError:
                    print(
                        "Couldn't detect any faces in {}'s image!".format(sample_name2)
                    )
                    continue
...
def face_recognition_fn(sample_face_encodings, sample_face_names):
    ...
    while True:
        ...
        for (top_coord, right_coord, bottom_coord, left_coord), face_encoding in zip(
            face_locations, face_encodings
        ):
            face_matches = face_recognition.compare_faces(
                sample_face_encodings, face_encoding
            )
            ...
            if face_matches[best_match_ind]:
                ...
                mark_attendance(
                    date,
                    time,
                    display_name_id.split("_")[0],
                    display_name_id.split("_")[1],
                )
...
def mark_attendance(date, time, name, student_id):
    ...
    while True:
        try:
            with open(
                ...
                + "/Attendance_register_25_7_21.csv",
                "r",
                newline="",
            ) as file:
    ...
    with open(
        ...
        if (
            student_id_date != []
            and student_id_date[len(student_id_date) - 1][1] != date
            and student_id_date[len(student_id_date) - 1][1] != ""
            and csv_input[len(csv_input) - 1] != ["\n"]
        ):
            csv_input.append("\n")
